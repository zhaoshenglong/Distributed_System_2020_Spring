...
public class Mapper {
    ...
    public static void doMap(String jobName, int mapTask, String inFile, int nReduce, MapFunc mapF) {
        ...
        HashMap<Integer, ArrayList<KeyValue>> intermediateList = new HashMap<>();
        int targetReducer;

        fileContent = readFileString(inFile);

        keyValueList = mapF.map(inFile, fileContent);

        for (KeyValue item : keyValueList) {
            targetReducer = hashCode(item.key) % nReduce;
            intermediateList.computeIfAbsent(targetReducer, k -> new ArrayList<>());
            intermediateList.get(targetReducer).add(item);
        }

        for (int reducerId : intermediateList.keySet()) {
            dumpToFile(Utils.reduceName(jobName, mapTask, reducerId),
                    JSONArray.toJSONString(intermediateList.get(reducerId)));
        }
    }
